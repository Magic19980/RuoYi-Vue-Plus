...
create table if not exists dm_department (
    dept_id              bigint      not null comment '系统部门ID（sys_dept.dept_id）',
    status               varchar(20)     not null default 'ENABLED' comment '科室状态（ENABLED启用 DISABLED停用）',
    manager_user_id      bigint      default null comment '科室负责人用户ID',
...
create table if not exists dm_person_profile (
    ...
    ended_by            bigint      default null comment '结束服务操作人',
    end_reason          varchar(500)    default null comment '结束服务原因',
    version             bigint      default 0 comment '版本号',
    create_dept         bigint      default null comment '纳入日报的目标科室/创建部门',
    create_by           bigint      default null comment '创建者',
    create_time         datetime        default null comment '创建时间',
    update_by           bigint      default null comment '更新者',
    update_time         datetime        default null comment '更新时间',
...
create table if not exists dm_person_profile_event (
    id                  bigint      not null comment '主键',
    profile_id          bigint      not null comment '人员档案ID',
    user_id             bigint      not null comment '用户ID',
    dept_id             bigint      not null comment '服务科室ID',
...
create table if not exists dm_daily_report (
    ...
    dept_id            bigint      not null comment '填报部门ID',
    ...
    leave_id            bigint      default null comment '关联休假ID，人工修改后解除关联',
    version             bigint      default 0 comment '版本号',
    create_dept         bigint      default null comment '创建部门',
    create_by           bigint      default null comment '创建者',
    create_time         datetime        default null comment '创建时间',
...
create table if not exists dm_daily_calendar_override (
    ...
    user_id             bigint      default null comment '目标人员ID；为空表示全体成员生效',
    calendar_date       date            not null comment '例外日期',
    day_type            varchar(50)     not null comment '日期例外类型字典值（dm_date_exception）',
    need_report         tinyint      not null default 0 comment '是否需要填写日报（1是 0否）',
...
create table if not exists dm_department_document (
    ...
    current_oss_id        bigint      not null comment '当前文件OSS ID',
    ...
    current_file_size     bigint      default null comment '当前文件大小（字节）',
...
-- 协作社区：用于跨部门经验分享、问题求助和改进讨论；正式资料仍归档到资料库，提案仍进入SCORE流程。
create table if not exists dm_department_community_post (
    ...
    visibility          varchar(20)     not null default 'ALL' comment '可见范围（ALL全员 DEPT本部门）',
    dept_id             bigint      default null comment '发帖时部门快照',
    status              varchar(20)     not null default 'PUBLISHED' comment '状态（DRAFT草稿 PUBLISHED已发布 RESOLVED已解决 ARCHIVED已归档）',
    view_count          int         not null default 0 comment '浏览次数',
    like_count          int         not null default 0 comment '点赞数',
    comment_count       int         not null default 0 comment '评论数',
    favorite_count      int         not null default 0 comment '收藏数',
    accepted_comment_id bigint      default null comment '采纳的评论ID',
    create_dept         bigint      default null comment '创建部门',
    create_by           bigint      default null comment '创建者',
    create_time         datetime        default null comment '创建时间',
    update_by           bigint      default null comment '更新者',
    ...
    primary key (id),
    key idx_dm_community_post_feed (status, visibility, create_time),
    key idx_dm_community_post_type (post_type, status),
    key idx_dm_community_post_author (create_by, create_time),
    key idx_dm_community_post_dept (dept_id, status)
) engine=innodb comment='协作社区帖子';

create table if not exists dm_department_community_comment (
    id                  bigint      not null comment '主键',
    post_id             bigint      not null comment '帖子ID',
    parent_id           bigint      not null default 0 comment '父评论ID，0表示一级评论',
    content             varchar(2000)   not null comment '评论内容',
    status              varchar(20)     not null default 'ENABLED' comment '评论状态（ENABLED正常 DISABLED隐藏）',
    create_dept         bigint      default null comment '创建部门',
    create_by           bigint      default null comment '创建者',
    create_time         datetime        default null comment '创建时间',
    update_by           bigint      default null comment '更新者',
    ...
    del_flag            char(1)         not null default '0' comment '删除标志',
    primary key (id),
    key idx_dm_community_comment_post (post_id, create_time),
    key idx_dm_community_comment_parent (parent_id)
) engine=innodb comment='协作社区评论';

create table if not exists dm_department_community_reaction (
    id                  bigint      not null comment '主键',
    post_id             bigint      not null comment '帖子ID',
    user_id             bigint      not null comment '互动用户ID',
    reaction_type       varchar(20)     not null comment '互动类型（LIKE点赞 FAVORITE收藏）',
    create_dept         bigint      default null comment '创建部门',
    create_by           bigint      default null comment '创建者',
    create_time         datetime        default null comment '创建时间',
    update_by           bigint      default null comment '更新者',
    ...
    del_flag            char(1)         not null default '0' comment '删除标志',
    primary key (id),
    unique key uk_dm_community_reaction (post_id, user_id, reaction_type, del_flag),
    key idx_dm_community_reaction_post (post_id, reaction_type, del_flag)
) engine=innodb comment='协作社区点赞收藏';

create table if not exists dm_department_community_media (
    id                  bigint      not null comment '主键',
    post_id             bigint      not null comment '帖子ID',
    oss_id              bigint      not null comment 'OSS文件ID',
    ...
    original_name       varchar(255)    default null comment '原始文件名',
    file_suffix         varchar(20)     default null comment '文件后缀',
    content_type        varchar(100)    default null comment '媒体类型',
    file_size           bigint      default null comment '文件大小（字节）',
    ...
    del_flag            char(1)         not null default '0' comment '删除标志',
    primary key (id),
    unique key uk_dm_community_media (post_id, oss_id, del_flag),
    key idx_dm_community_media_post (post_id, sort_num),
    key idx_dm_community_media_oss (oss_id)
) engine=innodb comment='协作社区媒体附件';

create table if not exists dm_department_community_comment_media (
    id                  bigint      not null comment '主键',
    comment_id          bigint      not null comment '评论ID',
    oss_id              bigint      not null comment 'OSS文件ID',
    media_type          varchar(20)     not null default 'IMAGE' comment '媒体类型（IMAGE图片）',
    original_name       varchar(255)    default null comment '原始文件名',
    file_suffix         varchar(20)     default null comment '文件后缀',
    content_type        varchar(100)    default null comment '媒体类型',
    file_size           bigint      default null comment '文件大小（字节）',
    ...
    del_flag            char(1)         not null default '0' comment '删除标志',
    primary key (id),
    unique key uk_dm_community_comment_media (comment_id, oss_id, del_flag),
    key idx_dm_community_comment_media_comment (comment_id, sort_num),
    key idx_dm_community_comment_media_oss (oss_id)
) engine=innodb comment='协作社区评论图片附件';

create table if not exists dm_department_community_report (
    id                  bigint      not null comment '主键',
    post_id             bigint      not null comment '帖子ID',
    reporter_user_id    bigint      not null comment '举报人用户ID',
    reason              varchar(500)   not null comment '举报原因',
    status              varchar(20)    not null default 'PENDING' comment '处理状态（PENDING待处理 REJECTED驳回 TAKEN_DOWN下线）',
    ...
    del_flag            char(1)         not null default '0' comment '删除标志',
    primary key (id),
    unique key uk_dm_community_report_pending (post_id, reporter_user_id, status),
    key idx_dm_community_report_status (status, create_time),
    key idx_dm_community_report_post (post_id)
) engine=innodb comment='协作社区举报';
...
create table if not exists dm_weekly_report (
    ...
    report_count        int         not null default 0 comment '日报条数',
    required_user_count int         not null default 0 comment '应填人数',
    filled_user_count   int         not null default 0 comment '已填人数',
    missing_user_count  int         not null default 0 comment '缺报人数',
...
create table if not exists dm_work_order_import_batch (
    id                    bigint      not null comment '导入批次主键',
    source_file_name      varchar(255)    not null comment '原始文件名',
    oss_id                bigint      default null comment '原始PDF OSS ID',
    source_period_start   date            default null comment 'PDF推断的业务周期开始',
    source_period_end     date            default null comment 'PDF推断的业务周期结束',
    page_count            int         not null default 0 comment 'PDF页数',
    record_count          int         not null default 0 comment '识别到的记录数',
    parsed_record_count   int         not null default 0 comment '已生成候选记录数',
    pending_record_count  int         not null default 0 comment '待人工确认记录数',
...
create table if not exists dm_work_order (
    id                    bigint      not null comment '主键',
    dept_id               bigint      not null comment '业务所属部门ID',
    ...
    resolution_minutes    int         default null comment '处理时长（分钟）',
    ...
    source_batch_id       bigint      default null comment 'PDF导入批次ID',
    source_file_name      varchar(255)    default null comment '来源文件名',
    source_page           int         default null comment '来源PDF页码',
...
create table if not exists dm_work_order_detail (
    id                    bigint      not null comment '明细主键',
    work_order_id         bigint      not null comment '人工单主记录ID',
    source_page           int         default null comment '来源PDF页码',
    sequence_no           int         not null comment 'PDF表格序号',
    request_dept          varchar(100)    default null comment '申请部门',
...
create table if not exists dm_operation_record (
    id                    bigint      not null comment '主键',
    dept_id               bigint      not null comment '业务所属部门ID',
    project_id            bigint      default null comment '绑定的科室项目ID',
    ...
    response_minutes      int         default null comment '响应耗时（分钟）',
    processing_minutes    int         default null comment '处理耗时（分钟）',
...
create table if not exists dm_operation_system (
    ...
    downtime_minutes      int         default null comment '系统停机时间（分钟）',
...
create table if not exists dm_five_why (
    ...
    analyst_user_id             bigint      default null comment '分析人系统用户ID，用于任务统计',
    ...
    before_oss_id               bigint      default null comment '改善前图片OSS ID',
    after_oss_id                bigint      default null comment '改善后图片OSS ID',
    ...
    reviewer_user_id            bigint      default null comment '实际审核人系统用户ID',
    reviewed_at                 datetime        default null comment '审核完成时间',
    review_file_oss_id          bigint      default null comment '提交审核时生成的Excel OSS ID',
    review_file_name            varchar(255)    default null comment '提交审核时生成的Excel文件名',
    revision_no                 int         not null default 0 comment '提交版本号',
    submitted_at                datetime        default null comment '提交审核时间',
    submitted_by                bigint      default null comment '提交审核操作人',
    confirm_comment             varchar(1000)   default null comment '现场确认意见',
    confirmer_user_id           bigint      default null comment '现场确认人用户ID',
...
create table if not exists dm_score_category (
    id                  bigint      not null comment '主键',
    parent_id           bigint      not null default 0 comment '父分类ID，0表示提案大类',
    category_name       varchar(500)    not null comment '分类名称',
    category_level      tinyint      not null default 1 comment '分类层级（1大类 2小类）',
    sort_num            int         not null default 0 comment '排序号',
    status              varchar(20)     not null default 'ENABLED' comment '状态（ENABLED启用 DISABLED停用）',
    remark              varchar(500)    default null comment '备注',
    version             bigint      default 0 comment '版本号',
...
create table if not exists dm_score_proposal (
    id                          bigint      not null comment '主键',
    dept_id                     bigint      not null comment '所属部门ID',
    main_category_id            bigint      default null comment '提案大类ID',
    sub_category_id             bigint      default null comment '提案小类ID',
    proposer_user_id            bigint      default null comment '提议人系统用户ID，用于任务统计',
    ...
    before_oss_id               bigint      default null comment '改进前图片OSS ID',
    after_oss_id                bigint      default null comment '改进后图片OSS ID',
...
create table if not exists dm_score_proposal_review_task (
    id                  bigint      not null comment '主键',
    proposal_id         bigint      not null comment 'SCORE提案ID',
    dept_id             bigint      not null comment '科室ID',
    revision_no         int         not null comment '提案版本号',
    stage               varchar(20)     not null comment '阶段（REVIEW审核/CONFIRM现场确认）',
    assignee_user_id    bigint      not null comment '任务处理人',
    ...
    completed_by        bigint      default null comment '完成操作人',
...
create table if not exists dm_review_rule (
    ...
    reviewer_user_id            bigint      not null comment '主审核人用户ID',
    backup_reviewer_user_id     bigint      default null comment '备用审核人用户ID',
...
create table if not exists dm_department_task_rule (
    ...
    required_count              int         not null default 1 comment '周期内要求完成次数',
    deadline_day                int         not null default 0 comment '周期内截止日序号，0表示周期最后一天',
    ...
    remind_hours                int         not null default 24 comment '截止前多少小时提醒',
    effective_start             date            default null comment '生效开始日期',
    effective_end               date            default null comment '生效结束日期',
    status                      varchar(20)     not null default 'ENABLED' comment '状态（ENABLED启用 DISABLED停用）',
    remark                      varchar(1000)   default null comment '备注',
...
create table if not exists dm_department_task_assignment (
    id                          bigint      not null comment '主键',
    rule_id                     bigint      not null comment '任务规则ID',
    dept_id                     bigint      not null comment '科室ID',
    user_id                     bigint      not null comment '被分配成员ID',
...
create table if not exists dm_department_task_completion (
    id                  bigint      not null comment '主键',
    instance_id         bigint      not null comment '周期实例ID',
    task_type           varchar(30)     not null comment '任务类型',
    source_id           bigint      not null comment '来源业务记录ID',
...
insert ignore into sys_menu values(1761400000000003140, '协作社区', 1761400000000003000, 10, 'community', 'department/community/index', '', 'N', 'Y', 'C', '0', '0', 'department:community:list', 'message', '', '', 1761000000000000103, 1761100000000000001, sysdate(), null, null, '跨部门经验分享、问题求助和改进讨论');
insert ignore into sys_menu values(1761400000000003141, '社区查询', 1761400000000003140, 1, '', '', '', 'N', 'Y', 'F', '0', '0', 'department:community:query', '#', '', '', 1761000000000000103, 1761100000000000001, sysdate(), null, null, '');
insert ignore into sys_menu values(1761400000000003142, '发布内容', 1761400000000003140, 2, '', '', '', 'N', 'Y', 'F', '0', '0', 'department:community:add', '#', '', '', 1761000000000000103, 1761100000000000001, sysdate(), null, null, '');
insert ignore into sys_menu values(1761400000000003143, '编辑内容', 1761400000000003140, 3, '', '', '', 'N', 'Y', 'F', '0', '0', 'department:community:edit', '#', '', '', 1761000000000000103, 1761100000000000001, sysdate(), null, null, '');
insert ignore into sys_menu values(1761400000000003144, '删除内容', 1761400000000003140, 4, '', '', '', 'N', 'Y', 'F', '0', '0', 'department:community:remove', '#', '', '', 1761000000000000103, 1761100000000000001, sysdate(), null, null, '');
insert ignore into sys_menu values(1761400000000003145, '评论互动', 1761400000000003140, 5, '', '', '', 'N', 'Y', 'F', '0', '0', 'department:community:comment', '#', '', '', 1761000000000000103, 1761100000000000001, sysdate(), null, null, '');
insert ignore into sys_menu values(1761400000000003146, '点赞收藏', 1761400000000003140, 6, '', '', '', 'N', 'Y', 'F', '0', '0', 'department:community:interact', '#', '', '', 1761000000000000103, 1761100000000000001, sysdate(), null, null, '');
insert ignore into sys_menu values(1761400000000003147, '社区管理', 1761400000000003140, 7, '', '', '', 'N', 'Y', 'F', '0', '0', 'department:community:moderate', '#', '', '', 1761000000000000103, 1761100000000000001, sysdate(), null, null, '');
insert ignore into sys_menu values(1761400000000003148, '举报内容', 1761400000000003140, 8, '', '', '', 'N', 'Y', 'F', '0', '0', 'department:community:report', '#', '', '', 1761000000000000103, 1761100000000000001, sysdate(), null, null, '');
insert ignore into sys_role_menu values (1761300000000000001, 1761400000000003140);
insert ignore into sys_role_menu values (1761300000000000001, 1761400000000003141);
insert ignore into sys_role_menu values (1761300000000000001, 1761400000000003142);
insert ignore into sys_role_menu values (1761300000000000001, 1761400000000003143);
insert ignore into sys_role_menu values (1761300000000000001, 1761400000000003144);
insert ignore into sys_role_menu values (1761300000000000001, 1761400000000003145);
insert ignore into sys_role_menu values (1761300000000000001, 1761400000000003146);
insert ignore into sys_role_menu values (1761300000000000001, 1761400000000003148);
insert ignore into sys_role_menu (role_id, menu_id) select distinct role_id, 1761400000000003140 from sys_role_menu where menu_id = 1761400000000003000;
insert ignore into sys_role_menu (role_id, menu_id) select distinct role_id, 1761400000000003141 from sys_role_menu where menu_id = 1761400000000003000;
insert ignore into sys_role_menu (role_id, menu_id) select distinct role_id, 1761400000000003142 from sys_role_menu where menu_id = 1761400000000003000;
insert ignore into sys_role_menu (role_id, menu_id) select distinct role_id, 1761400000000003143 from sys_role_menu where menu_id = 1761400000000003000;
insert ignore into sys_role_menu (role_id, menu_id) select distinct role_id, 1761400000000003144 from sys_role_menu where menu_id = 1761400000000003000;
insert ignore into sys_role_menu (role_id, menu_id) select distinct role_id, 1761400000000003145 from sys_role_menu where menu_id = 1761400000000003000;
insert ignore into sys_role_menu (role_id, menu_id) select distinct role_id, 1761400000000003146 from sys_role_menu where menu_id = 1761400000000003000;
insert ignore into sys_role_menu (role_id, menu_id) select distinct role_id, 1761400000000003148 from sys_role_menu where menu_id = 1761400000000003000;

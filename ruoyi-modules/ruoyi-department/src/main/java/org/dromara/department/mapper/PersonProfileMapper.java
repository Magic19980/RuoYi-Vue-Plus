...
public interface PersonProfileMapper extends BaseMapperPlus<PersonProfile, PersonProfileVo> {
    ...
    @Select({
        ...
        "d.indonesian_name as dept_indonesian_name, u.employee_no, post.job_title, post.job_title_indonesian_name,",
        ...
        "left join (select sup.user_id, group_concat(distinct sp.post_name order by sp.post_sort, sp.post_id separator '、') as job_title, "
            + "group_concat(distinct coalesce(nullif(sp.post_indonesian_name, ''), sp.post_name) order by sp.post_sort, sp.post_id separator '、') as job_title_indonesian_name "
            + "from sys_user_post sup join sys_post sp on sp.post_id = sup.post_id and sp.status = '0' and sp.del_flag = '0' "
            + "group by sup.user_id) post on post.user_id = p.user_id",
        "where p.del_flag = '0'",
        ...
        "<if test='bo.jobTitle != null and bo.jobTitle != \"\"'> and (post.job_title like concat('%', #{bo.jobTitle}, '%') "
            + "or post.job_title_indonesian_name like concat('%', #{bo.jobTitle}, '%')) </if>",
    ...
    @Select({
        ...
        "and (post.job_title like concat('%', #{bo.jobTitle}, '%') "
            + "or post.job_title_indonesian_name like concat('%', #{bo.jobTitle}, '%'))",
        "</if>",
        "order by u.user_name",
        "</script>"
    })
